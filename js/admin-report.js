...
var _RPT_API = 'https://eventpro-fxfv.onrender.com/api';
...
function _rptApiFetch(url, headers) {
...
function _renderMeta(result) {
  ...
  var d = result.value;
  _rptSetText('metaEvent',     d.eventName     || (d.event && d.event.name)         || 'All Events');
  _rptSetText('metaDate',      d.eventDate     ? _rptFmtDate(d.eventDate)            : new Date().toLocaleDateString('en-GB', {day:'numeric',month:'long',year:'numeric'}));
  _rptSetText('metaOrganizer', d.organizerName || (d.organizer && d.organizer.name) || 'Admin');
}
...
var _STAT_CONFIG = [
  { cls: 'attendees', label: 'Total Users',    keys: ['totalUsers',   'total_users',   'totalAttendees'], icon: _iconAttendees() },
  { cls: 'vip',       label: 'Total Events',   keys: ['totalEvents',  'total_events',  'vipTickets'],     icon: _iconVip()       },
...
function _renderStats(result) {
  ...
  list.innerHTML = _STAT_CONFIG.map(function (cfg) {
    var raw   = d ? _rptPick(d, cfg.keys) : null;
      ...
      datasets: [
        {
          ...
          borderWidth: 2.5, pointRadius: 4, pointHoverRadius: 6, tension: 0.4, fill: true,
        },
        {
          label: 'Checked-In', data: checkedIn,
          borderColor: '#00BFA5', backgroundColor: 'rgba(0,191,165,0.06)',
          borderWidth: 2.5, pointRadius: 4, pointHoverRadius: 6, tension: 0.4, fill: true,
        },
      ],
    },
    options: {
      ...
      plugins: {
        legend: {
          ...
          labels: { font: { family: 'Poppins', size: 12 }, boxWidth: 12, boxHeight: 12, padding: 16 },
        },
        tooltip: {
...
function _exportCSV() {
  ...
  if (!rows.length) {
    _rptShowToast('No data to export.', 'error');
    return;
  }
  ...
  _rptDownloadFile('summary-report.csv', lines.join('\n'), 'text/csv');
  _rptShowToast('CSV exported successfully.', 'success');
}
...
function _exportPDF() {
  if (typeof window.jspdf === 'undefined') {
    _rptShowToast('PDF library not loaded yet. Please wait.', 'error');
    return;
  }
  ...
  _rptShowToast('PDF downloaded successfully.', 'success');
}
...
function _rptDownloadFile(filename, content, mimeType) {
